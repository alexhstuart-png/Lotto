...
Extract every game line printed on the ticket. Each game has exactly 7 main
numbers (each 1-35) followed by 1 separate Powerball number (1-20) — the
Powerball is usually labelled "PB" or printed after the mains. Game lines are
usually labelled A, B, C, D...
...
export async function scanTicketImage({ imageBase64, mediaType }) {
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new Error('Ticket scanning is not set up yet — add an ANTHROPIC_API_KEY in the Netlify environment variables');
  }
  ...
  for (const g of parsed.games.slice(0, 50)) {
    ...
    if (v.ok) games.push({ numbers: v.numbers, powerball: v.powerball });
    else rejected++;
  }
  if (games.length === 0) {
