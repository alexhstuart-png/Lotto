...
import { supabase, must, auditLog } from './db.mjs';
...
export async function saveResultsAndProcess({ drawId, numbers, powerball, divisions = null, source, actorId = null }) {
  ...
  const existing = must(
    await supabase().from('results').select('*').eq('draw_id', drawId).maybeSingle()
  );
  ...
  const ticket = must(
    await supabase().from('tickets').select('id').eq('draw_id', drawId).maybeSingle()
  );
  let hasWinner = false;
  if (ticket) {
    const games = must(
      await supabase()
        .from('games')
        .select('id, game_index, numbers, powerball')
  ...
  must(await supabase().from('draws').update({ status }).eq('id', drawId).select().single());
