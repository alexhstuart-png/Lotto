...
import { saveResultsAndProcess, reprocessDraw } from './results-pipeline.mjs';
...
export async function findDrawAwaitingResults() {
  ...
  for (const d of draws) {
    ...
    if (!firstComplete) firstComplete = d;
  }
  // Everything fetched: offer the most recent complete draw for an
  // idempotent reprocess (self-heals runs cut off mid-pipeline).
  return firstComplete ? { draw: firstComplete, needs: 'reprocess' } : null;
}
...
export async function fetchAndSaveLatestResults({ maxAttempts = 3, delayMs = 3000, markFailedOnError = true } = {}) {
  ...
  if (needs === 'reprocess') {
    // No network needed: finish/verify matching, winnings and status from
    // the stored result. Idempotent, so re-running is always safe.
    const processed = await reprocessDraw(draw.id);
    return { status: 'reprocessed', draw, hasWinner: processed.hasWinner, autoRecorded: processed.autoRecorded ?? 0 };
  }
