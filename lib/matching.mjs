...
/**
 * Match one game against the official result. Works for standard games
 * (7 mains) and System 8-20 entries (8-20 mains): only 7 winning mains
 * exist, so the intersection is at most 7 and the division reported is the
 * BEST line the system entry contains — a system entry also collects lower
 * divisions, which is why prize amounts are always admin-confirmed.
...
export function matchGame(game, result) {
  ...
  // PowerHit plays every Powerball 1-20, so it always hits the winning one.
  const powerballMatched = game.powerhit === true || game.powerball === result.powerball;
