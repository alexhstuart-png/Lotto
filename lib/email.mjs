...
export function ticketPublishedEmail({ memberName, drawDate, games, costCents, kittyCents, balanceCents = null, weeklyChargeCents = 2500 }) {
  ...
  // Plain-english balance line: how much they have left, or how much they owe.
  let balanceBlock = '';
  if (balanceCents !== null) {
    const redBox = (inner) =>
      `<p style="background:rgba(231,111,81,0.12);border:1px solid #e76f51;border-radius:8px;padding:10px 14px;">${inner}</p>`;
    if (balanceCents < 0) {
      balanceBlock = redBox(`You <strong style="color:#e76f51;">owe ${formatMoney(-balanceCents)}</strong> — please square up before next Thursday to stay in the draw.`);
    } else if (balanceCents <= weeklyChargeCents) {
      balanceBlock = redBox(`You've only got <strong style="color:#e76f51;">${formatMoney(balanceCents)}</strong> left — that's your last draw's worth. Top up before next Thursday to stay in.`);
    } else {
      const drawsLeft = weeklyChargeCents > 0 ? Math.floor(balanceCents / weeklyChargeCents) : null;
      balanceBlock = `<p>You've got <strong style="color:#35c46f;">${formatMoney(balanceCents)}</strong> left in the kitty${drawsLeft ? ` — good for ${drawsLeft} more draw${drawsLeft === 1 ? '' : 's'}` : ''}.</p>`;
    }
  }
  return {
