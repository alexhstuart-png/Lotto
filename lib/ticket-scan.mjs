...
import { validateSflGame } from './setforlife.mjs';
...
const SFL_PROMPT = `This photo shows an Australian "Set for Life" lottery ticket.

Extract every game line printed on the ticket. A STANDARD game has exactly 7
numbers (each 1-44); a SYSTEM entry (marked "System 8"/"Sys 8" etc., up to
System 15) has that many numbers. There is NO Powerball on this game. Game
lines are usually labelled A, B, C, D... Count each line's numbers carefully
and include ALL of them.

Respond with ONLY a JSON object, no other text, in exactly this shape:
{"games":[{"numbers":[n,n,n,n,n,n,n]}, ...],"notes":"..."}

Rules:
- Include a game ONLY if you can read all of its numbers clearly and
  unambiguously; otherwise OMIT the whole game and explain in "notes".
- Do not guess. Missing a line is fine; a wrong number is not.
- "notes": short summary, empty string if nothing to note.`;

/**
 * @param {string} [opts.gameType] 'powerball' (default) or 'setforlife'
 * @returns {Promise<{games: object[], notes: string, rejected: number}>}
 * @throws on configuration, transport, or unusable-response errors — callers
 *         surface the message to the admin.
 */
export async function scanTicketImage({ imageBase64, mediaType, gameType = 'powerball' }) {
  ...
  const response = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: gameType === 'setforlife' ? SFL_PROMPT : PROMPT },
      ],
    }],
  });
  ...
  for (const g of parsed.games.slice(0, 50)) {
    if (gameType === 'setforlife') {
      const v = validateSflGame(g);
      if (v.ok) games.push({ numbers: v.numbers });
      else rejected++;
    } else {
