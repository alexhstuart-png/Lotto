    ...
    if (hasWinner && divisions) {
      ...
      if (known.length) {
        ...
        if (error) {
          ...
          // Single batched insert keeps this well inside function time limits.
          must(
...
/**
 * Re-run matching + auto-winnings from the stored result. Fully idempotent
 * (winnings dedupe on the unique index; status update converges), so it
 * safely completes any run that was cut off mid-pipeline (e.g. a function
 * timeout after dividends saved but before winnings were recorded).
 */
export async function reprocessDraw(drawId) {
  const result = must(
    await supabase().from(T('results')).select('numbers, powerball, divisions')
      .eq('draw_id', drawId).maybeSingle()
  );
  if (!result) return { status: null, hasWinner: false, autoRecorded: 0 };
  return processMatchingAndWinnings(drawId, result.numbers, result.powerball, result.divisions);
}
