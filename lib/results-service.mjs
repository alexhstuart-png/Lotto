...
async function fetchLatestFromTheLott(productFilter) {
...
export const theLottProvider = {
  ...
  async fetchLatestSetForLifeResult() {
    return parseTheLottSflResponse(await fetchLatestFromTheLott('SetForLife'));
  },
};
...
export function parseTheLottSflResponse(body) {
  ...
  const draw = body.DrawResults.find((d) => /setforlife/i.test(String(d.ProductId ?? '')));
  if (!draw) throw new Error('No Set for Life draw in response');
