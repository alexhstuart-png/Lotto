...
export function ticketPublishedEmail({ memberName, drawDate, games, costCents, kittyCents }) {
  const gameRows = games
    ...
    .join('');
  return {
    subject: `This week's Powerball ticket is in — draw ${esc(drawDate)}`,
    html: wrap(`
      <p>Hi ${esc(memberName)},</p>
      <p>This week's ticket has been purchased for the Thursday night Powerball draw on <strong>${esc(drawDate)}</strong>. Good luck!</p>
      <table style="border-collapse:collapse;margin:12px 0;">${gameRows}</table>
      <p>Ticket cost: <strong>${formatMoney(costCents)}</strong><br>
         Kitty balance: <strong>${formatMoney(kittyCents)}</strong></p>
    `),
  };
}
